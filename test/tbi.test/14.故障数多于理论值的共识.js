...
describe('故障数多于理论值的共识', () => {
    before(async ()=> {
        ...
        //通过SDK连接1号节点，为3号节点转账 - 注：这并不意味着1号节点需要运行，仅仅是为了满足3号节点转账所需的前置条件
        await radmin.execute('tx.send', [address, 150000000]);
        await radmin.execute('miner.generate.admin', [1]);
        await radmin.wait(1000);
    })
