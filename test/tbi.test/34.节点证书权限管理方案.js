...
describe('节点证书权限管理方案', () => {
    ...
    it('不符合权限管理策略的操作', async () => {
        ...
        let ret = await remoteOperator.execute('sys.alliance.create', ['bookmansoft', notes[1].id, notes[1].alliance, `${notes[1].inner}:${notes[1].tcp}`]);
    ...
    it('符合权限管理策略的操作', async () => {
        ...
        let ret = await remote.execute('sys.alliance.create', ['bookmansoft', notes[1].id, notes[1].alliance, `${notes[1].inner}:${notes[1].tcp}`]);
        //断言操作成功
        assert(!ret.error);
    });
    ...
    it('符合权限管理策略的操作', async () => {
        ...
        let ret = await remote.execute('sys.alliance.create', ['bookmansoft', notes[2].id, notes[2].alliance, `${notes[2].inner}:${notes[2].tcp}`]);
        //断言操作成功
        assert(!ret.error);
    });
    ...
    it('不符合权限管理策略的操作', async () => {
        ...
        let ret = await remoteOperator.execute('sys.alliance.delete', [notes[1].id, notes[1].alliance]);
    ...
    it('符合权限管理策略的操作', async () => {
        ...
        let ret = await remote.execute('sys.alliance.delete', [notes[1].id, notes[1].alliance]);
        //断言操作成功
        assert(!ret.error);
    ...
    it('查看节点证书列表', async () => {
        ...
        let ret = await remote.execute('sys.alliance.list', []);
        assert(!ret.error);
