...
describe('链间互操作（同构）', () => {
  ...
  before(async ()=>{
    ...
    for(let i=0; i<env.transaction; i++) {
      ...
      await remoteB.wait(1000);

      ret = await remoteB.execute('balance.all', [env.bob.name]);
      console.log(`${env.bob.name}'s balance`, ret.result.confirmed);
    }
    ...
    ret = await remoteB.execute('address.create', [env.alice.name]);
    assert(ret.code == 0);
